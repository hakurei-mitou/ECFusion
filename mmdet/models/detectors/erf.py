...
class ERF(BaseDetector):
    def __init__(self,
        ...
        if self.fusion:
            ...
            if multi_level_fusion is not None:
    ...
    def extract_feat(self, batch_inputs: Tensor, c_batch_inputs: Tensor):
        ...
        if self.fusion:
            e_feats = self.event_backbone(batch_inputs)
            c_feats = self.color_backbone(c_batch_inputs)
        elif self.only_color:
            c_feats = self.color_backbone(c_batch_inputs)
        elif self.only_event:
            e_feats = self.event_backbone(batch_inputs)

        return e_feats, c_feats
    ...
    def loss(self,
        ...
        final_loss = ({
            f'loss_cls' : losses['loss_cls'],
            f'loss_bbox' : losses['loss_bbox']
        })
    ...
    def predict(self,
        ...
        results_list = self.bbox_head.predict(feats, batch_data_samples, rescale=True)
