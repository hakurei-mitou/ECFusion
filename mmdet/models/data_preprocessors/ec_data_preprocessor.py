...
class EC_ImgDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False, need_norm = False) -> Union[dict, list]:
...
@MODELS.register_module()
class EC_DetDataPreprocessor(EC_ImgDataPreprocessor):
    ...
    def __init__(self,
        ...
        self.other_detector = flow['other_detector']

    def forward_single(self, data: dict, training: bool = False, need_norm = False) -> dict:
        
        batch_pad_shape = self._get_pad_shape(data)
        data = super().forward(data=data, training=training, need_norm=need_norm)
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if self.only_event:
            inputs, data_samples = self.forward_single(data, training, need_norm=False)
        elif self.only_color:
            # to swap
            data['inputs'] = data.pop('c_inputs')
            c_inputs, data_samples = self.forward_single(data, training, need_norm=True)
        elif self.fusion:
            # event
            inputs, data_samples = self.forward_single(data, training, need_norm=False)
            ...
            # image
            # to device
            c_inputs = self.cast_data(data['c_inputs'])
            ...
            # into batch
            c_inputs = stack_batch(c_inputs, pad_size_divisor=1, pad_value=.0)
            
            # to float
            c_inputs = c_inputs.float()

            # only meta information of events be retained
        if self.other_detector:
            if self.only_event:
                inputs = inputs
            elif self.only_color:
                inputs = c_inputs
            ...
            return {'inputs': inputs, 'data_samples': data_samples, 'c_inputs': c_inputs}
