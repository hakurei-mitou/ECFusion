...
class MultiLevelEICA(nn.Module):
    def __init__(self,
            dims = [512, 1024, 2048]
        ):
        super().__init__()
        self.eica_list = nn.ModuleList()
        for dim in dims:
            self.eica_list.append(
                EventImage_ChannelAttentionTransformerBlock(dim=dim)
            )

    def forward(self, e_feats, c_feats):

        feats = []
        for i, eica in enumerate(self.eica_list):
            feats.append(eica(c_feats[i], e_feats[i]))
