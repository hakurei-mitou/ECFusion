...
flow=dict(only_event=True, only_color=False, fusion=False,
          only_heatmap=False, one_level=True)
freeze=dict(freeze_event=True, event_checkpoint='work_dirs/ECFusion_E_DSEC_test_4/epoch_30.pth')
num_queries = 20
...
model = dict(
    ...
    heatmap_head=dict(
        ...
        feat_channels=128,
        train_cfg=None,
        # this topk only be used when only heatmap for EC_CenterNetHead
    ...
    decoder_layer=dict(
        ...
        train_cfg=dict(
            assigner=dict(
                ...
                match_costs=[
                    # dict(type='ClassificationCost', weight=0.15),
                    dict(type='FocalLossCost', weight=0.15),
                    dict(type='BBoxL1Cost', weight=0.25, box_format='xywh'),
                    dict(type='IoUCost', iou_mode='giou', weight=0.25)
                ])),
...
custom_imports = dict(imports=['mmdet.datasets.transforms.LoadEventAndImage'], allow_failed_imports=False)
backend_args = None
